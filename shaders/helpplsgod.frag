...
SDF map(vec3 p) {
    ...
    ground.distance -= bumpMapping(u_ground_disp,p, p - 0.01, ground.distance, 0.01, 1);
    ground.distance += 0.01;
    ground.id = 10.0;
...
Material getMaterial(SDF object, vec3 p, inout vec3 normal) {
  ...
  case 6:
    ...
    break;
  case 7:
    material.albedo = vec3(1.0, 0.84, 0.0); // Gold color
    material.metallic = 1.0;                // Fully metal
  ...
  case 8:
    ...
    break;
  case 10:
    material.albedo = triPlanar(u_ground, p, normal).rgb;
    break;
  default:
...
void getCurrentHit(vec3 rayOrigin, vec3 rayDirection,
  ...
  if (object.distance < MAX_DIST) {
    ...
    hitInfo.material =
        getMaterial(object, rayOrigin + rayDirection * object.distance, hitInfo.normal);
